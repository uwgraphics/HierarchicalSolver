...
int main(int argc,char *argv[])
{
    ...
    n = topology.n;
    ia_B = new index_type[n+1];
    ja_B = new index_type[topology.offsets[n]];
    index_type idx = 0;
    //std::cout<<"offsets"<<std::endl;
    for (int e:topology.offsets) {
        ia_B[idx++] = (index_type)e;
        //std::cout<<idx-1<<" "<<e<<std::endl;
    }
    idx = 0;
    //std::cout<<"rows"<<std::endl;
    for (int e:topology.row) {
        ja_B[idx++] = (index_type)e;
        //        std::cout<<idx-1<<" "<<e<<std::endl;
    }

    {
       // fill in data
        ...
        a_B = new T[ia_B[n]*d*d];
        for (index_type j=0; j<n; j++)
            for (index_type ii=ia_B[j]; ii<ia_B[j+1]; ii++) {
                index_type i=ja_B[ii];
                const T_INDEX& index = reordering.coordinates(j+1);
                const T_INDEX& neighbor_index = reordering.coordinates(i+1);
                for (int v=0; v<d*d; v++)
                    a_B[ii*d*d+v] = K_matrix[index(1)-1][index(2)-1][index(3)-1][neighbor_index(1)-index(1)+1][neighbor_index(2)-index(2)+1][neighbor_index(3)-index(3)+1][v/d][v%d];
            }
...
/* -------------------------------------------------------------------- */
    ...
    iparm[36]=d; /* Use Blocked CSR format for input, need to have iparm[12] = 0 */
