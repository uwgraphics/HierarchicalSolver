...
class THREADED_UNIFORM_GRID:public MPI_GRID<T_GRID>
{
    ...
    template<typename T_SPECIAL> struct WORKAROUND_STRUCT{};
public:
    ...
    template<class T2, class T3> void Average_Common_Face_Data_Helper(ARRAY<T2,FACE_INDEX<TV::dimension> >& data, WORKAROUND_STRUCT<T3>& ws) const;
    template<class T2> void Average_Common_Face_Data_Helper(ARRAY<T2,FACE_INDEX<TV::dimension> >& data, WORKAROUND_STRUCT<int>& ws) const;
    template<class T2> void Average_Common_Face_Data_Helper(ARRAY<T2,FACE_INDEX<TV::dimension> >& data, WORKAROUND_STRUCT<bool>& ws) const;
