    ...
    :MPI_GRID<T_GRID>(local_grid_input,number_of_ghost_cells_input,true,processes_per_dimension,periodic_input,0),tid(tid_input),buffers(buffers_input)
{
    ...
    std::stringstream ss;ss<<"number of processes = "<<number_of_processes<<std::endl;LOG::filecout(ss.str(),tid);
#endif
    ...
    ss1<<"coordinates = "<<coordinates<<std::endl;
    LOG::filecout(ss1.str(),tid);
#endif
...
{   
    ...
    for(int axis=1;axis<=T_GRID::dimension;axis++)
        ss<<"boundaries "<<axis<<" = "<<boundaries(axis)(coordinates[axis])<<" to "<<boundaries(axis)(coordinates[axis]+1)<<std::endl;
    ss<<"topology = "<<process_grid.Domain_Indices()<<std::endl;
    ss<<"process ranks = \n"<<process_ranks;
    LOG::filecout(ss.str(),tid);
#endif
}
//#####################################################################
...
//#####################################################################
// Function Synchronize_Ghost_Cells
//#####################################################################
template<class T_GRID> void THREADED_UNIFORM_GRID<T_GRID>::
Synchronize_Ghost_Cells(int& ghost_cells) const
{
    ...
    pthread_mutex_unlock(lock);
    pthread_barrier_wait(barr);
    for(int i=1;i<=buffers.m;i++) ghost_cells=max(*(int*)(&buffers(i).buffer(1)),ghost_cells);
    pthread_barrier_wait(barr);
    if(tid==1) buffers.m=0;
    pthread_barrier_wait(barr);
#endif
...
Average_Common_Face_Data(ARRAY<T2,FACE_INDEX<TV::dimension> >& data) const
{
    ...
    Average_Common_Face_Data_Helper(data,ws);
#endif
}
//#####################################################################
// Function Average_Common_Face_Data_Helper
//#####################################################################
template<class T_GRID> template<class T2,class T3> void THREADED_UNIFORM_GRID<T_GRID>::
Average_Common_Face_Data_Helper(ARRAY<T2,FACE_INDEX<TV::dimension> >& data, WORKAROUND_STRUCT<T3>& ws) const
{
#ifdef USE_PTHREADS
    ...
    for(int n=1;n<=regions(1).m;n++)if(side_neighbor_ranks(n)!=-1){int axis=(n-1)/2+1;
        ...
        for(FACE_ITERATOR iterator(local_grid,regions(axis)(n),axis);iterator.Valid();iterator.Next()){
            ...
            data(iterator.Full_Index())=data(iterator.Full_Index())*(T)0.5+local_data(iterator.Full_Index())*(T)0.5;}}
    pthread_barrier_wait(barr);
    if(tid==1) buffers.m=0;
    pthread_barrier_wait(barr);
#endif
...
//#####################################################################
// Function Average_Common_Face_Data_Helper - For T2=int
//#####################################################################
template<class T_GRID> template<class T2> void THREADED_UNIFORM_GRID<T_GRID>::
Average_Common_Face_Data_Helper(ARRAY<T2,FACE_INDEX<TV::dimension> >& data, WORKAROUND_STRUCT<int>& ws) const
{
#ifdef USE_PTHREADS
    ARRAY<ARRAY<RANGE<TV_INT> > > regions(T_GRID::dimension);
    for(int axis=1;axis<=T_GRID::dimension;axis++)Find_Boundary_Regions(regions(axis),Face_Sentinels(axis),false,RANGE<VECTOR<int,1> >(0,0),false);
    ...
    for(int n=1;n<=regions(1).m;n++)if(side_neighbor_ranks(n)!=-1){int axis=(n-1)/2+1;
        ...
        for(FACE_ITERATOR iterator(local_grid,regions(axis)(n),axis);iterator.Valid();iterator.Next()){
            local_data.data(axis).Unpack(buffers(index).buffer,position,iterator.Face_Index());
            data(iterator.Full_Index())=(data(iterator.Full_Index())+local_data(iterator.Full_Index()))/2;}}
    pthread_barrier_wait(barr);
    if(tid==1) buffers.m=0;
    pthread_barrier_wait(barr);
#endif
...
//#####################################################################
// Function Average_Common_Face_Data_Helper - For T2=bool
//#####################################################################
template<class T_GRID> template<class T2> void THREADED_UNIFORM_GRID<T_GRID>::
Average_Common_Face_Data_Helper(ARRAY<T2,FACE_INDEX<TV::dimension> >& data, WORKAROUND_STRUCT<bool>& ws) const
{
#ifdef USE_PTHREADS
    ARRAY<ARRAY<RANGE<TV_INT> > > regions(T_GRID::dimension);
    for(int axis=1;axis<=T_GRID::dimension;axis++)Find_Boundary_Regions(regions(axis),Face_Sentinels(axis),false,RANGE<VECTOR<int,1> >(0,0),false);
    ...
    for(int n=1;n<=regions(1).m;n++)if(side_neighbor_ranks(n)!=-1){int axis=(n-1)/2+1;
        ...
        for(FACE_ITERATOR iterator(local_grid,regions(axis)(n),axis);iterator.Valid();iterator.Next()){
            local_data.data(axis).Unpack(buffers(index).buffer,position,iterator.Face_Index());
            data(iterator.Full_Index())=data(iterator.Full_Index())||local_data(iterator.Full_Index());}}
    pthread_barrier_wait(barr);
    if(tid==1) buffers.m=0;
    pthread_barrier_wait(barr);
#endif
...
#define INSTANTIATION_HELPER(T,T_GRID,d)      \
    ...
    template void THREADED_UNIFORM_GRID<T_GRID>::Exchange_Boundary_Face_Data(GRID_ARRAYS_POLICY<T_GRID>::FACE_ARRAYS::REBIND<int>::TYPE&,const int) const; \
    ...
    template void THREADED_UNIFORM_GRID<T_GRID>::Average_Common_Face_Data(GRID_ARRAYS_POLICY<T_GRID>::FACE_ARRAYS::REBIND<int>::TYPE&) const; \
