...
public:
    ...
    void Set_Threaded_Boundary(RANGE<TV_INT>& domain,ARRAY<bool,FACE_INDEX<TV::dimension> >& psi_N){PHYSBAM_NOT_IMPLEMENTED();};

    void Solve_Threaded(RANGE<TV_INT>& domain,const GRID<TV>& threaded_grid,const T time=0){PHYSBAM_NOT_IMPLEMENTED();};
