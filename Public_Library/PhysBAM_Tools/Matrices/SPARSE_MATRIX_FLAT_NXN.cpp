...
Positive_Diagonal_And_Nonnegative_Row_Sum(const T tolerance) const
{
    ...
    for(int i=1;i<=n;i++){
        if((*this)(i,i)<=0){
            ...
            std::stringstream ss;
            ss<<"diagonal entry "<<i<<" contains nonpositive element: "<<(*this)(i,i)<<std::endl;
            LOG::filecout(ss.str());
#endif
            return false;}
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
            std::stringstream ss;
            ss<<"sum of row "<<i<<" is negative: "<<sum<<std::endl;
            LOG::filecout(ss.str());
#endif
            return_value=false;}}
    return return_value;
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
    std::stringstream ss;
    for(int i=1;i<=n;i++)ss<<offsets(i+1)-offsets(i)<<" ";ss<<std::endl;
    LOG::filecout(ss.str());
#endif
}
//#####################################################################
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
    std::stringstream ss;
    for(int i=1;i<=n;i++)if(Element_Present(row,i))ss<<"Col: "<<i<<" Val: "<<(*this)(row,i)<<",  ";
    ss<<std::endl;
    LOG::filecout(ss.str());
#endif
}
