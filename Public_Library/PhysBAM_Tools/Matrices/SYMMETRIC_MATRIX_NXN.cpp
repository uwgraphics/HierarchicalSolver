...
Jacobi_Solve_Eigenproblem(ARRAY<VECTOR<int,2> >& givens_pairs,ARRAY<VECTOR<T,2> >& givens_coefficients,const T tolerance,const int max_iterations)
{
    ...
    for(int iteration=1;iteration<=max_iterations;iteration++){
        ...
        t=(q>0)?(T)1/(q+(T)sqrt((double)(q*q+(T)1))):(T)1/(q-(T)sqrt((double)(q*q+(T)1)));
        c=(T)1/(T)sqrt((double)(t*t+(T)1));
        s=c*t;
...
In_Place_Cholesky_Factorization(MATRIX_MXN<T>& L)
{
    ...
    for(int j=1;j<=n;j++){ // for each column
        ...
        L(j,j)=(T)sqrt((double)(Element_Lower(j,j)));
...
//#####################################################################
template <class T> void SYMMETRIC_MATRIX_NXN<T>::
Resize(const int n_new)
{
    if(n_new==n) return;int new_size = (n_new*n_new+n_new)/2;
    T* x_new=new T[new_size];for(int t=0;t<new_size;t++) x_new[t]=(T)0;
    for(int i=0;i<n;i++) for(int j=i;j<n;j++) x_new[(i*n_new+j)-((1<<i)-1)]=(*this)(i+1,j+1);
    delete[] x;x=x_new;n=n_new;size=new_size;
}
//#####################################################################
// Function Similarity_Transform
//#####################################################################
template <class T> template <class T_MATRIX> void SYMMETRIC_MATRIX_NXN<T>::
Similarity_Transform(SYMMETRIC_MATRIX_NXN<T>& result,const MATRIX_BASE<T,T_MATRIX>& basis)
{
    assert(Columns()==basis.Rows());
    result.Resize(basis.Columns());
    //TODO: clean up by using direct indexing
    VECTOR_ND<T> temp,temp2;temp.Resize(basis.Rows());temp2.Resize(basis.Columns());
    for(int i=1;i<=basis.Columns();i++) {
        for(int j=1;j<=basis.Rows();j++) {temp(j)=basis(j,i);}
        temp2=basis.Derived().Transposed()*(this->operator*(temp));
        for(int j=1;j<=i;j++){result(j,i)=temp2(j);}
    }
}
//#####################################################################
#define INSTANTIATION_HELPER(T) \
    template class SYMMETRIC_MATRIX_NXN<T>; \
    template void SYMMETRIC_MATRIX_NXN<T>::Similarity_Transform<MATRIX_MXN<T> >(SYMMETRIC_MATRIX_NXN<T>&,const MATRIX_BASE<T,MATRIX_MXN<T> >&);
