    ...
    if(read_static_variables){
        ...
        for(int i=1;i<=m;i++){
            ...
            if(SEGMENTED_CURVE_2D<T>* segmented_curve=dynamic_cast<SEGMENTED_CURVE_2D<T>*>(structure)){
                ...
                free_particles_indirect_arrays(i)=new INDIRECT_ARRAY<ARRAY_VIEW<TV> >(deformable_geometry_collection->particles.X,fp->nodes);
                free_particles_objects(i)=new OPENGL_FREE_PARTICLES<TV>(*deformable_geometry_collection,*free_particles_indirect_arrays(i),color_map->Lookup(color_map_index--));}
