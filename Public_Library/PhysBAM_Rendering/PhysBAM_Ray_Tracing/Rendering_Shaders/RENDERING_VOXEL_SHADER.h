...
public:
    ...
    using VOLUMETRIC_SHADER<T>::Inverse_Isotropic_Phase_Direction;
    ...
    TV Inverse_Phase_Direction(const TV& incoming,const int tid=1)
    {if(isotropic_scattering) return Inverse_Isotropic_Phase_Direction(incoming,tid);
    else return Inverse_Henyey_Greenstein_Phase_Direction(incoming,phase_function_g,tid);}
...
//#####################################################################
    TV Attenuate_Color(const RENDERING_RAY<T>& ray,const RENDERING_OBJECT<T>& object,const TV& color,const int tid=1) PHYSBAM_OVERRIDE;
    TV Attenuate_Light(const RENDERING_RAY<T>& ray,const RENDERING_OBJECT<T>& object,const RENDERING_LIGHT<T>& light,const TV& light_color,const int tid=1) PHYSBAM_OVERRIDE;
    bool Scatter_Photon_Ray(const RENDERING_OBJECT<T>& object,RENDERING_RAY<T>& ray,TV& photon_power,const typename PHOTON_MAP<T>::PHOTON_MAP_TYPE type,const T fixed_step_size,const int tid=1) PHYSBAM_OVERRIDE;
    TV Attenuate_Photon(const RENDERING_RAY<T>& ray, const RENDERING_OBJECT<T>& object, const TV& photon_power, bool& should_throw,const int tid=1) PHYSBAM_OVERRIDE {return TV(photon_power);}
//#####################################################################
};
}
#endif
