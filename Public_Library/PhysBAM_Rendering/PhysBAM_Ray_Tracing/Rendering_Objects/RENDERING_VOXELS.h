...
public:
    ...
    using RENDERING_OBJECT<T>::Object_Space_Point;using RENDERING_OBJECT<T>::Object_Space_Ray;
    ...
    void Preprocess_Efficiency_Structures(RENDER_WORLD<T>& world,const int tid=1) PHYSBAM_OVERRIDE
    {if(precompute_single_scattering) Precompute_Light_Data(true,world,tid);}

//#####################################################################
    void Precompute_Light_Data(bool use_fast_precomputation,RENDER_WORLD<T>& world,const int tid=1);
