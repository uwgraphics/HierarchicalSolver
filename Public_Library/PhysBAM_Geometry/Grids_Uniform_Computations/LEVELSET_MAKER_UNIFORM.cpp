...
Compute_Level_Set(TRIANGULATED_SURFACE<T>& triangulated_surface,GRID<TV>& grid,ARRAY<T,TV_INT>& phi,ARRAY<TV,TV_INT>* velocity)
{
    ...
    if(use_orthogonal_vote){
        ...
        if(keep_only_largest_inside_region){
            ...
            if(verbose) ss<<"Keeping only largest inside region (max region size = "<<max_region_size<<")... "<<std::flush;
#endif
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
        if(verbose) ss<<"(got "<<number_of_colors<<" colors)... "<<std::endl;
#endif
        if(number_of_colors==1 && !phi_offset){ // there is only one color. check if the whole domain is inside or outside then return
            if(triangulated_surface.Inside(grid.X(1,1,1)))phi.Fill(-FLT_MAX);else phi.Fill(FLT_MAX);if(verbose) LOG::filecout(ss.str());return false;}
        // TODO: put this back if you need it
        /*if(write_debug_data){
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
            if(verbose){ss<<"Path between "<<path_start_node<<" and "<<path_end_node<<" "<<(path_exists?"exists":"doesn't exist")<<std::endl;}
#endif
...
#ifndef COMPILE_WITHOUT_READ_WRITE_SUPPORT
                if(verbose) ss<<"Majority of corners are inside -- flipping sign!"<<std::endl;
#endif
    ...
    if(verbose) LOG::filecout(ss.str());
    return true;
}
//#####################################################################
